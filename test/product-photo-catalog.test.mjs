...
test("the adult-diaper catalog contains all 116 local product photos", () => {
  assert.equal(PRODUCT_PHOTOS.length, 116);
  assert.deepEqual(
    Object.fromEntries(
    ...
    { "カミ商事": 41, "ユニ・チャーム": 39, "第一衛材": 36 },
  );
  assert.equal(new Set(PRODUCT_PHOTOS.map((photo) => photo.id)).size, 116);
  assert.equal(new Set(PRODUCT_PHOTOS.map((photo) => photo.asset)).size, 116);
